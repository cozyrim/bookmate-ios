...
- 기능별 서비스의 갱신 분기를 공통 `APIClient`로 통합. 원래 요청의 재시도는 한 번으로 제한.
- `actor`라도 `await` 중 다른 호출이 들어올 수 있으므로, 상태 보호와 중복 작업 방지는 별도로 설계할 필요. 진행 중인 `Task`를 공유하는 방식 선택.
- 인증 실패와 일시적인 네트워크 오류를 구분해야 불필요한 로그아웃을 줄일 수 있음. 실제 로그아웃률 감소는 측정하지 않아 수치로 제시하지 않음.
