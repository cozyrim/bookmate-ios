...
- 로그아웃 시 로컬 토큰을 즉시 삭제하고, 캡처한 refresh token으로 서버 폐기 요청. 연결 실패나 앱 종료 시 서버 폐기까지 보장하지는 못함.
- 갱신 응답 저장 직전에 현재 refresh token 대조. 로그아웃·다른 계정 로그인 뒤 도착한 응답은 저장하지 않고 해당 회전 토큰도 서버 폐기 요청.
- 이전 세션의 진행 중 API 응답은 새 로그인 상태에 반영하지 않음. `401`만 인증 만료로 해석하고 `429`·일시적 오류는 토큰 보존.
- 시뮬레이터 테스트 5개: 정상 갱신, 401·429 구분, 로그아웃 뒤 늦은 갱신, 새 로그인 보호. 서버 실제 폐기와 연동하려면 이 코드가 포함된 앱 버전 출시 필요.
