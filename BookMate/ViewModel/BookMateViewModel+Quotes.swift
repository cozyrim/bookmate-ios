...
//
// 문장 조회, 저장, 수정, 삭제를 관리한다.

import Foundation

extension BookMateViewModel {
    // 특정 책에 저장된 문장 목록을 서버에서 불러온다.
    func loadQuotes(bookId: UUID) async {
        ...
        do {
            ...
            quoteLoadErrorMessage = "문장을 불러오지 못했습니다."
            DebugLogger.log("문장 조회 실패:", error)
        }
    }

    // 메모리에 있는 문장 중 특정 책에 속한 문장만 반환한다.
    func savedQuotes(for bookId: UUID) -> [Quote] {
        quotes.filter { $0.bookId == bookId }
    }

    // 새 문장을 서버에 저장하고 로컬 목록 맨 앞에 추가한다.
    func saveQuote(bookId: UUID, text: String, page: Int?, memo: String?) async -> Bool {
        ...
        do {
            ...
            operationErrorMessage = "문장 저장에 실패했습니다."
            showToast("문장 저장에 실패했어요.", style: .error)
            DebugLogger.log("문장 저장 실패:", error)
            return false
        }
    }

    // 기존 문장을 서버에 수정 요청하고 로컬 목록을 갱신한다.
    func updateQuote(_ quote: Quote) async -> Bool {
        ...
        do {
            ...
            if handleUnauthorizedIfNeeded(error) { return false }
            operationErrorMessage = "문장 수정에 실패했습니다."
            showToast("문장 수정에 실패했어요.", style: .error)
            DebugLogger.log("문장 수정 실패:", error)
            return false
        }
    }

    // 서버에서 문장을 삭제하고 로컬 목록에서도 제거한다.
    func deleteQuote(_ quote: Quote) async -> Bool {
        ...
        do {
            ...
            if handleUnauthorizedIfNeeded(error) { return false }
            operationErrorMessage = "문장 삭제에 실패했습니다."
            showToast("문장 삭제에 실패했어요.", style: .error)
            DebugLogger.log("문장 삭제 실패:", error)
            return false
        }
    }
}
